...
function cargarDatosCotizacion(result) {
    var arrCotizacion = [];
    arrCotizacion.push({
        "lblContenido": kony.store.getItem('descripcionPlanSeleccionado')
    });
    arrCotizacion.push({
        "lblContenido": "Precio estimado: " + cotizadorCalculo.txtPrecio.text
    });
    arrCotizacion.push({
        "lblContenido": 'Plazo a ' + cotizadorCalculo.sliderPlazos.selectedValue + ' meses'
    });
    arrCotizacion.push({
        "lblContenido": 'Mensualidad: ' + result.smensualidad
    });
    arrCotizacion.push({
        "lblContenido": 'Comision por apertura: ' + result.sComisionApertura
    });
    arrCotizacion.push({
        "lblContenido": 'Total de anticipo: ' + result.sEnganche
    });
    ...
    if (esPlanAlternativo == 'true') {
        arrCotizacion.push({
            "lblContenido": 'Valor final: ' + result.svalorfinal
        });
    }
    ...
    if (result.errmsg != undefined) {
        ...
        if (result.codigo != null && result.codigo != undefined && result.codigo == 1) {
            if (result.clase != undefined) {
                var arrFichaTecnica = [];
                arrFichaTecnica.push({
                    "lblContenidoFicha": "Combustible " + "\n" + result.combustible
                });
                arrFichaTecnica.push({
                    "lblContenidoFicha": "Potencia (Hp@rpm)" + "\n" + result.potencia
                });
                arrFichaTecnica.push({
                    "lblContenidoFicha": "Torque (Nm@rpm)" + "\n" + result.torque
                });
                arrFichaTecnica.push({
                    "lblContenidoFicha": "Transmisión" + "\n" + result.transmision
                });
                arrFichaTecnica.push({
                    "lblContenidoFicha": "Tracción" + "\n" + result.motorTraccion
                });
                arrFichaTecnica.push({
                    "lblContenidoFicha": "N° de Cilindros / Disposición" + "\n" + result.cilindrosDisposicion
                });
                arrFichaTecnica.push({
                    "lblContenidoFicha": "Cilindrada" + "\n" + result.cilindrada
                });
                arrFichaTecnica.push({
                    "lblContenidoFicha": "Consumo Combinado" + "\n" + result.consumoCombinado
                });
