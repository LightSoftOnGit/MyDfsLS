 		...
 		if (result.errmsg != undefined) { 			
	 		...
	 		if (result.codigo != null && result.codigo != undefined && result.codigo == 1) { 		 						
    			if (result.lstPlanes != undefined && result.lstPlanes.length>0) {
    				...
    				for(i=0;i<result.lstPlanes.length;i++){    					
    					if(idPlan==idFinanciamientoFlex){		
    						if(result.lstPlanes[i].esPlanAlternativo==true){
    							arrPlanes.push({"lblId":result.lstPlanes[i].id,"lblContent":result.lstPlanes[i].descripcion,"lblPlanAlternativo":result.lstPlanes[i].esPlanAlternativo.toString()});	
    						}
    					} else {
    						if(result.lstPlanes[i].esPlanAlternativo!=true){
    							arrPlanes.push({"lblId":result.lstPlanes[i].id,"lblContent":result.lstPlanes[i].descripcion,"lblPlanAlternativo":result.lstPlanes[i].esPlanAlternativo.toString()});	
    				...
    				cotizadorPlanes2.segmentCalculadora.widgetDataMap={lblId:"lblId",lblContent:"lblContent",lblPlanAlternativo:"lblPlanAlternativo"};
...
function limpiarControlesPlanes2(){
	try{
		var arrVacio=[];
		cotizadorPlanes2.segmentCalculadora.widgetDataMap={lblId:"lblId",lblContent:"lblContent",lblPlanAlternativo:"lblPlanAlternativo"};
		arrVacio.push({"lblId":"","lblContent":"","lblPlanAlternativo":""});
